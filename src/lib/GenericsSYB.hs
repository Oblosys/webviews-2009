...
mkWebNodeMap :: (Typeable db, Data wv) => wv -> WebNodeMap db
...
assignIdsFromList :: (Data db, MapWebView db (WebView db)) => [Id] -> WebView db -> WebView db  
assignIdsFromList allIds x = (snd $ everywhereAccum assignId freeIds x)
